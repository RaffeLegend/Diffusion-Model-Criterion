#!/usr/bin/env python
"""
测试多层级 PDE 一致性 Criterion

用法:
    python test_multilevel.py --dataset-dir ./dataset --max-samples 100
"""
...
def test_method(method, image_paths, labels, config_base, model_manager=None):
    """测试单个方法"""
    from criteria.multilevel_criterion import MultiLevelCriterion
    ...
    criterion = MultiLevelCriterion(config, model_manager)
    ...
    if len(scores) == 0:
        return {'method': method, 'auc': 0.5, 'direction': 'N/A', 'real_mean': 0, 'fake_mean': 0}, None
...
def main():
    ...
    parser.add_argument("--batch-size", type=int, default=2)  # CLIP 需要更多显存
    parser.add_argument("--num-noise", type=int, default=4)   # 减少噪声数
    parser.add_argument("--time-frac", type=float, default=0.01)
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--methods", type=str, nargs='+', 
                       default=["latent", "clip", "hybrid", "kappa_only", "cosine_latent"])
    args = parser.parse_args()
    
    print("=" * 60)
    print("Multi-Level PDE Consistency Test")
    print("=" * 60)
    ...
    print("\n" + "=" * 80)
    print("SUMMARY - Multi-Level PDE Consistency")
    print("=" * 80)
